<!DOCTYPE html>
    ...
    <head>
        <meta charset="utf-8">
          ...
          <title>Nội thất Hà Nội | Doanh thu</title>
  ...
  <style type="text/css">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Hóa đơn đã hoàn thành</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
      ...
      <form action="" method="POST">
          ...
          <button type="submit" name="xuat">Xuat</button>
      </form>
    </div>
          ...
          <table class="table table-bordered mt-3 .bg-light">
            ...
            <?php
                ...
                if(isset($_POST['xuat'])){
                    ...
                    $query=mysqli_query($conn,"SELECT DISTINCT orderid, user_id, status FROM tblorderdetails INNER JOIN tblorder ON tblorderdetails.orderid=tblorder.id WHERE status='2' AND tblorder.odate>='$tungay' AND tblorder.odate<='$denngay' ORDER BY orderid LIMIT $perRow, $rowsPerPage");
                }
                 else{
                    $query=mysqli_query($conn,"SELECT DISTINCT orderid, user_id, status FROM tblorderdetails INNER JOIN tblorder ON tblorderdetails.orderid=tblorder.id WHERE status='2'  ORDER BY orderid LIMIT $perRow, $rowsPerPage");
                }
               
                $totalRows = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tblorder WHERE status='2' "));
